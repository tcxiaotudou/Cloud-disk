...
func OnUserFileUploadFinished(userName, fileHash, fileName string, fileSize int64) bool {
	...
	if err != nil {
		fmt.Println("insert into tbl_user_file failed, err:", err)
		return false
	}
	defer stmt.Close()
...
func QueryUserFileMetas(username string, limit int) ([]UserFile, error) {
	...
	if err != nil {
		fmt.Println("query user file metas failed, error:", err)
		return nil, err
	}
	defer stmt.Close()
	rows, err := stmt.Query(username, limit)
	if err != nil {
		fmt.Println("exec query user file metas failed, error:", err)
		return nil, err
	}
